...
export default class Pizza {
  ...
  getTotalCost() {
    return this.getBaseCost() + this.toppingCodes.reduce((total, code) => total + TOPPINGS[code].cost, 0);
  }

  getDescription() {
    return `A ${this.size} pizza with ${this.toppingCodes.map(code => TOPPINGS[code].name).join(", ")}.`;
  }
}
